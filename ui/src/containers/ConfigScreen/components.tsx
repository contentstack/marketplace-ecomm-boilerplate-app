  ...
  return objValue?.options?.map((option: any) => {
    return (
      <Radio
        ...
        name={objKey}
        checked={
          configurationObject?.multi_config_keys?.[multiConfigurationDataID]?.[
            objKey
          ] === option?.label
        }
        label={option?.label}
...
export const CheckboxInputField: React.FC<CustomComponentProps> = function ({
  ...
  const handleOnChange = (e: any) => {
    const selectedOption = objValue?.options?.find(
      (key: any) => key?.label === e?.target?.name
    );

    let updatedValues;
    if (
      selectedValues?.some((item: any) => item?.label === selectedOption?.label)
    ) {
      updatedValues = selectedValues.filter(
        (item: any) => item?.label !== selectedOption?.label
      );
    } else {
