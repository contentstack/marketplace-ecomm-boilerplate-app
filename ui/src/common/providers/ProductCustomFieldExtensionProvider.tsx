...
const ProductCustomFieldExtensionProvider: React.FC<any> = function ({
  ...
  const initialLoad = async () => {
    if (location) {
      ...
      const retrievedAdvancedFieldConfigs = await location
          ?.fieldConfig;
        const currentLocale = await location?.entry
          ?.locale;
      ...
      const hasCsMetadataInfieldData=fieldData?.data?.some((obj: any)=>hasCsMetadataKeys(obj))
      if(hasCsMetadataInfieldData){
      }
      else{
        fieldData?.data?.forEach((product: { cs_metadata: { multi_config_name: any; }; }) => {
      ...
      if (fieldData?.data?.length) {
        if (
          ...
          transformedData = fieldData?.data?.reduce((result: { [x: string]: { id: any[]; }; }, product: { id: any; cs_metadata?: any; }) => {
            const { id, cs_metadata } = product;
          
            // Check if cs_metadata is defined and has multi_config_name
            if (cs_metadata && cs_metadata.multi_config_name !== undefined && cs_metadata.multi_config_name !== null) {
              ...
              console.warn(`cs_metadata or multi_config_name is undefined or null for product id: ${id}`);
  ...
  const fetchData = async (selectedIdsArray: any) => {
    ...
    if (Object.keys(selectedIdsArray)?.length && !isEmpty(appConfig)) {
      try {
        ...
        const results = await Promise.all(apiCalls.map((call: { apiCall: any }) => call.apiCall));
        ...
        results.forEach((res, index) => {
          ...
          if (res?.error) {
            ...
            const oldItem = oldData.find((item: any) => item.id === id) || { id, masterData: {} };
        ...
        const allIds = Array.from(new Set([...oldData.map((item: any) => item.id), ...combinedIds]));
        const finalData = allIds.map(id => {
          const foundInAggregated = combinedData.find((item: any) => item.id === id);
  ...
  const removeIdFromField = (removeId: any,multiConfigName:any) => {
    ...
    // } else {
      ...
      const updatedOldData=oldData?.filter((data:any)=>data?.id!==removeId)
